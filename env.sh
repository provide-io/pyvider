...
if [ $SYNC_EXIT_CODE -eq 0 ]; then
    ...
    echo "Check /tmp/pyvider_setup/sync.log for details"
    
    # Try to install just the project without dependencies first
    echo -n "Installing pyvider without dependencies..."
    uv pip install --no-deps -e . > /tmp/pyvider_setup/install_nodeps.log 2>&1 &
    INSTALL_PID=$!
    spinner $INSTALL_PID
    wait $INSTALL_PID
    if [ $? -eq 0 ]; then
        print_success "pyvider installed (no deps)"
    else
        print_error "Failed to install pyvider"
        return 1 2>/dev/null || exit 1
    fi
fi
