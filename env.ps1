# env.ps1 - pyvider Development Environment Setup
#
...
if (-not (Test-Path "pyproject.toml")) {
    ...
    Write-Host "Please run this script from the pyvider root directory"
    exit 1
}
...
$Profile = if ($env:PYVIDER_PROFILE) { $env:PYVIDER_PROFILE } else { "default" }
if ($Profile -eq "default") {
    $VenvDir = "workenv/pyvider_${TFOS}_${TFARCH}"
} else {
...
$LogDir = Join-Path $env:TEMP "pyvider_setup"
...
Write-Host "Installing pyvider in editable mode..." -NoNewline
try {
    & uv pip install --no-deps -e . 2>&1 | Out-File -FilePath (Join-Path $LogDir "install.log")
    Write-Success " pyvider installed"
}
catch {
...
Write-Host "`n$("pyvider development environment activated" | Write-Host -ForegroundColor Green)"
Write-Host "Virtual environment: $VenvDir"
Write-Host "Profile: $Profile"
Write-Host "`nUseful commands:"
Write-Host "  pyvider --help  # pyvider CLI"
