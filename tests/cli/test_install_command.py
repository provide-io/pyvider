...
class TestInstallCommandValidation:
    ...
    def test_install_accepts_soup_toml(self, tmp_path: Path) -> None:
        """Test that install command accepts soup.toml."""
        runner = CliRunner()
        with runner.isolated_filesystem(temp_dir=tmp_path):
            # Create a soup.toml file
            Path("soup.toml").write_text('[pyvider]\nname = "tofusoup"\n')
    ...
    def test_install_uses_custom_provider_name_from_pyvider_toml(self, tmp_path: Path) -> None:
        ...
        with runner.isolated_filesystem(temp_dir=tmp_path):
            ...
            with mock.patch("pyvider.cli.install_command.is_running_as_binary", return_value=False):
                ...
                # The context should have loaded "tofusoup" as provider name
                # Plugin dir path should contain "tofusoup" not "pyvider"
                # (This is tested more directly in test_context.py)

    def test_install_uses_custom_provider_name_from_soup_toml(self, tmp_path: Path) -> None:
        """Test that install uses custom provider name from soup.toml in paths."""
        runner = CliRunner()
        with runner.isolated_filesystem(temp_dir=tmp_path):
            # Create soup.toml with custom provider name
            Path("soup.toml").write_text('[pyvider]\nname = "tofusoup"\n')
            Path("VERSION").write_text("0.0.1108")
