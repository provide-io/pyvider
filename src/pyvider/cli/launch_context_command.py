#
# pyvider/cli/launch_context_command.py
#
"""
...
def launch_context_cmd(format: str, verbose: bool) -> None:
    ...
    if format.lower() == "json":
        ...
        click.secho(f"Executable Path: ", fg="cyan", bold=True, nl=False)
        click.secho(launch_context.executable_path, fg="white")

        click.secho(f"Python Executable: ", fg="cyan", bold=True, nl=False)
        click.secho(launch_context.python_executable, fg="white")

        click.secho(f"Working Directory: ", fg="cyan", bold=True, nl=False)
        click.secho(launch_context.working_directory, fg="white")

        click.secho(f"Terraform Invoked: ", fg="cyan", bold=True, nl=False)
        ...
        if verbose:
            ...
            for key, value in env_info.items():
                if key == "argv":
                    ...
                    click.secho(f"  PSPF Environment Variables:", fg="cyan")
...
def _show_method_specific_help(method):
    ...
    if method.value == "pspf_package":
        ...
        click.echo(
            "  This provider is running from a PSPF (Progressive Secure Package Format)"
        )
        ...
        click.echo(
            "  This provider is running from an editable install (pip install -e)."
        )
