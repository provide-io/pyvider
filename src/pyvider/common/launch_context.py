#
# pyvider/common/launch_context.py
#
"""
...
def _detect_launch_method(
    executable_path: str, python_executable: str
) -> tuple[LaunchMethod, Dict[str, Any]]:
    """
...
def _is_pspf_launch(executable_path: str, python_executable: str) -> bool:
    ...
    if any(
        indicator in executable_path.lower()
        for indicator in ["terraform-provider-", "pspf"]
    ):
...
def _is_editable_install(executable_path: str) -> bool:
    ...
    try:
        ...
        if any(
            env_dir in str(exe_path)
            for env_dir in [".venv", "venv", "conda", "anaconda"]
        ):
            return True
...
def _analyze_cache_structure(python_path: pathlib.Path) -> Dict[str, Any]:
    ...
    try:
        if cache_dir.exists():
            structure["contents"] = [
                item.name for item in cache_dir.iterdir() if item.is_dir()
