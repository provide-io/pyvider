...
- [x] Run `lint_binary="$PWD/dist/$(uname -s | tr '[:upper:]' '[:lower:]')_$(uname -m | sed 's/x86_64/amd64/;s/aarch64/arm64/')/terraform-provider-pyvider_v$(cat VERSION)"` followed by `make test-conformance-binary PYVIDER_CONFORMANCE_PSP="$lint_binary"`; expect the existing 87-test packaged suite plus the new lint suite pass against that same explicit checksum. Never use `make test-conformance` for coordinated-stack verification because it rebuilds the binary from the ordinary lock.
- [x] Commit with message `test: prove seven lint paths in packaged provider`.

Approved provider commits: `169fc4f6a99212b240d40d1fbe4fc27d5b8c3d61`, `bcb7f58dab5412eac05057974c03588b91a1c43b`, `8ed48814f3001539efd386d44c480e952f4ce8cf`, `2d3bb0e3588f5ecef4f96ce6f6ef2939ee255128`, `7516af9da7a35745d93f1fb1349154dd27f3f3a3`. Approved component test-fixture commit: `17c74a09cd813e74ed520cbaed5c296ba1e016c6`. Independent spec review: approved after adding real file/environment precedence, packaged fail-open, exact RPC parameterization, and sanitized CLI failures. Independent code-quality review: approved after strengthening per-RPC proof contracts, provider-source provenance, subprocess cleanup, and atomic artifact publication. Flavorpack 0.5.3 does not list Python distributions in its raw provenance JSON, so the approved implementation preserves that output and separately inventories the actual wheels slot. Fresh evidence: 33 build tests, 38 focused packaged-provider tests, 125 full explicit-binary conformance tests, and 424 component tests passed; Ruff, mypy, and diff checks passed. Final pre-tracking artifact SHA-256: `6fd855550c13f4fa971887900e2ea697e536b47f204666c2d88514e68cd5be35`.
...
- [x] Add a docs test asserting the guide names both exact fixtures, both artifacts, v1.13.0-beta1, the four-vs-seven capability boundary, all reproduction targets, and these exact upstream links: `https://github.com/opentofu/opentofu/blob/main/rfc/20260406-linting.md`, `https://github.com/opentofu/opentofu/issues/4310`, `https://github.com/opentofu/opentofu/pull/4337`, and `https://github.com/opentofu/opentofu/releases/tag/v1.13.0-beta1`.
- [x] Run `uv run pytest tests/proof/test_provider_linting_docs.py -q` and observe missing documentation failure.
- [x] Document local coordinated build, OpenTofu JSON suite, TofuSoup direct suite, cast generation, manifest verification, and CI artifact retrieval. Say Pyvider's API is supported; say OpenTofu calls its built-in feature experimental; say ordinary warnings are the temporary compatibility bridge.
- [x] Run `uv run pytest tests/proof/test_provider_linting_docs.py -q && uv run mkdocs build --strict`; expect pass.
- [x] Reuse `lint_binary` from Task 8 and run `make test-conformance-binary PYVIDER_CONFORMANCE_PSP="$lint_binary"`, `make test-linting-opentofu-binary PYVIDER_CONFORMANCE_PSP="$lint_binary"`, and `uv run pytest tests/proof -q`; expect pass without any `flavor pack` rebuild.
- [x] Commit with message `docs: explain packaged linting proof`.
- [x] Push the reviewed Pyvider and components feature branches to `gh-origin`, capture their exact public commit SHAs with `git rev-parse HEAD`, then push the provider feature branch to `gh-origin`. Set `provider_sha` to the just-pushed provider HEAD and `dispatch_after` to the current UTC timestamp. Dispatch the already-default-branch workflow exactly with:
...
- [x] Replace `provider-linting.cast` and `provider-linting-proof.json` with the verified files from that exact run and commit only those two files as `chore: record final provider linting proof`. Because a tracked manifest cannot contain the hash of the commit that contains itself, enforce the non-self-referential invariant: the manifest's provider source SHA must equal this artifact-only commit's first parent, and `git diff --name-only HEAD^ HEAD` must list only those two artifacts. Record both the provider source SHA and artifact commit SHA for the site.

Approved provider commits: `695046b9e732adba8728bcce76c98e872b5cf29c`, `84b5576c2ce7f9a735e92ce0e8ca236b4f8c2b3a`, `70e65383e0986241e685b4845b4a63634593463a`, `1822f4b5b99f35b24b2082f927549a069cbcf994`, and artifact-only commit `1e222053f5bc0525c140110dd1872091d00bba92`. Independent spec and quality reviews approved the documentation, coordinated-source portability, exact selector language, publication wording, ordinary-vs-coordinated conformance selection, and final CI artifact invariants. The first dispatch, run `34818848712`, exposed that ordinary protocol jobs were collecting the coordinated-only lint module; systematic diagnosis led to the narrow reviewed `--ignore=tests/conformance/test_provider_linting.py` fix while the dedicated target retained all 141 conformance nodes. Fresh run [`34820452404`](https://github.com/provide-io/terraform-provider-pyvider/actions/runs/34820452404) uniquely matched the post-fix source and completed 23/23 jobs successfully. The downloaded artifact verified 7/7 and exactly matched the committed files. Provider source: `1822f4b5b99f35b24b2082f927549a069cbcf994`; Pyvider: `d9dd6b692c7aaefbf2bee5ed8a6c57dc1f09bc09`; components: `17c74a09cd813e74ed520cbaed5c296ba1e016c6`. Final cast SHA-256: `f1060bcb595006fef8c26ebea89710d47eb17a8337efaa952a3d32be35df606b`; manifest SHA-256: `c6e3676b509f0b3f3eeee377d22b7b89435629edd57733aee60d0d2ef18f3850`; CI binary SHA-256: `b45b4edbd15d8a373064650dea07d24f0ca08f511536d2d637740c1bd544d2b6`; downloaded build-provenance SHA-256: `5beb29e6834aa67619ce88cfbfa7d96fe5e2ab03e8c878c5a93e127734e6a632`. The artifact commit changes exactly the cast and manifest and has the recorded provider source as its first parent.
